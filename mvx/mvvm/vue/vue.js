...
(
    ...
    function getFunctionName(definition){
        var matches = definition.match(functionName);
        return matches[1];
    }

    function Print_Trace(arg){
        var _caller = arg.callee.caller;
        var _callee = arg.callee;
        alert(getFunctionName(_caller.toString()) + " is calling " + getFunctionName(_callee.toString()))
    }
    //function Print_Trace(arg){}
    ...
    function Debug_Vue(obj, path){
        ...
        for (var i=0; i<_len; i++){
            ...
            if(typeof obj[_keys[i]] === 'object'){
                Debug_Vue(obj[_keys[i]], _path)
            }else{
                alert(_path + " : " + obj[_keys[i]]);
            }
        }
...
// explicitness and function inlining
    ...
    //     return  str.replace(hyphenateRE, function(c){return '-' + c.toLowerCase()})
    // }
    ...
    function parsePath (path) {
        ...
        // keypath
        // app.$watch('$data.visible', function (newVal, oldVal) {
    ...
    {
        // if has console, then typeof console === 'object'
...
// detect devtools
     ...
     *
     * Every member in data option object and props option object has a one.
     * And in a component, member in props option object will get dealt with
     * before data option object.
     *
     * `this` is Dep instance
     */

    var Dep = function Dep () {
        this.id = uid++;
        this.subs = []; //subscriber,or say Watcher.
    };
    ...
    //Dep's target is a watcher instance.
    // `this` is instance of Dep
    Dep.prototype.depend = function depend () {
        if (Dep.target) {
            Dep.target.addDep(this);
           // Dep.target.addDep(this,"I am just for test");
        }
    };
    ...
    Dep.prototype.notify = function notify () {
        ...
        for (var i = 0, l = subs.length; i < l; i++) {
            //subs[i].update();
            subs[i].update("I am in Dep.prototype.notify calling subscriber's update");
        }
    };
    ...
    // `target` is a property directly defined on the Dep Function object.
    //  not on each the Dep instance.
    Dep.target = null;
    var targetStack = [];

    // `target `is a watcher instance.
    function pushTarget (_target) {
        //If Dep.target is not null, then we should save it.
    ...
    [
        ...
        .forEach(function (method) {
            ...
            def(arrayMethods, method, function mutator () {
                ...
                var ob = this.__ob__;
                // hold the new elements to be inserted in.
                var inserted;
                switch (method) {
                    case 'push':
                        inserted = args;
    ...
    Observer.prototype.walk = function walk (obj) {
        ...
        for (var i = 0; i < keys.length; i++) {
            defineReactive$$1(obj, keys[i], obj[keys[i]]); //object
        }
    };
...
// helpers
    ...
    function observe (value, asRootData) {
        ...
        //Print_Trace(arguments)
        // var observe_value = isObject(value) ? Object.keys(value) : value;
        // alert("observe: " + observe_value)
        if (!isObject(value)) {
            return
        }
        var ob;
        if (hasOwn(value, '__ob__') && value.__ob__ instanceof Observer) {
            //If the value already has one, then return the existing the observer instance.
            ob = value.__ob__;
        } else if (
            !isServerRendering() &&
            ...
            //Create a brand new observer instance for a value
    ...
    function defineReactive$$1 (
        ...
        //For non-object val, the childOb(obsever instance) is `undefined`.
        var childOb = observe(val);
        //alert("key: " + key +  "\'s childOb: " + childOb)

        Object.defineProperty(obj, key, {
            enumerable: true,
            configurable: true,
            get: function reactiveGetter () {
                // If no getter is given, then we access val through closure.
                var value = getter ? getter.call(obj) : val;

                // Dep.target 表示当前的watcher正在调用get()方法收集依赖
                // data中的成员如果没有在HTML中出现,那么默认不会有watcher去收集依赖.
                if (Dep.target) {
                    alert("The current watcher id : " + Dep.target.id + " collect dependecy: " + key)
                    dep.depend();
                    if (childOb) {
                        childOb.dep.depend();
                    }
            ...
            set: function reactiveSetter (newVal) {
                ...
                alert(key + "'s set is called with newVal: \'" +  newVal + "\' and oldVal: \'" + value +
                    "\', I am in defineReactive$$1");

                /* eslint-disable no-self-compare */
                if (newVal === value || (newVal !== newVal && value !== value)) {
                    //alert("No actual change!")
                    return
                }
     ...
     * already exist.
     *
     * vm.$set(target,key,value) or global Vue.set
     */
     ...
     *
     *  vm.$delete(target, key) or global Vue.delete
     */
    ...
    /**
     * Hooks and props are merged as arrays (strategies).
     */
    function mergeHook (
        parentVal,
        childVal
    ...
    /**
     * Other object hashes (strategies).
     */
    strats.props =
        strats.methods =
...
// is needed to cater to all possible types of children values.
    ...
    function initEvents (vm) {
        ...
        //alert(listeners===undefined ? "null" : Object.keys(listeners))
        if (listeners) {
    ...
    function lifecycleMixin (Vue) {
        ...
        Vue.prototype._update = function (vnode, hydrating) {
            ...
            alert("calling _update in lifecycleMixin")
            var vm = this;
            if (vm._isMounted) {
    ...
    var Watcher = function Watcher (
        ...
        //default cb: function noop () {}
        this.cb = cb;
        this.id = ++uid$2; // uid for batching
        this.active = true;
        this.dirty = this.lazy; // for lazy watchers

        //虽然deps可以有多个,但是一个watcher,callback函数的只有一个
        this.deps = [];
       ...
       // alert("callback  "  + cb  + "  id: " + this.id);
        ...
        this.value = this.lazy
            ? undefined
            : this.get("Watcher's get() in Watcher constructor");
         // : this.get();
        //alert(this.value === undefined ? "undefined" : Object.keys(this.value) + ", I am in Watcher")
    };
    ...
    Watcher.prototype.get = function get () {
        ...
        //alert(this instanceof Watcher + " I am in Watcher.prototype.get");
        //alert(arguments[0] + ", I am in Watcher.prototype.get")
        //alert("I am going to pushTarget with watcher id: " + this.id)

        pushTarget(this);
        var value;
        var vm = this.vm;
        if (this.user) {
            alert("calling this.user getter")
            try {
        ...
        //alert("I am going to popTarget with watcher id: " + this.id)
        popTarget();
        this.cleanupDeps();

        //alert(value === undefined ? "undefined" : Object.keys(value) + " I am in Watcher.prototype.get")
        return value
    };

    /**
     *  Add a dependency to this directive.
     *  Watcher ---> addDep  dep -->varible
     *  Dep ---> addSub      sub -->watcher
     */
    Watcher.prototype.addDep = function addDep (dep) {
        //alert(arguments[1] + " I am in Watcher.prototype.addDep")
        var id = dep.id;
    ...
    Watcher.prototype.cleanupDeps = function cleanupDeps () {
        ...
        //alert(this$1.id + " I am in  Watcher.prototype.cleanupDeps")
    ...
    Watcher.prototype.update = function update () {
        ...
        //alert(arguments[0] + " --> I am in Watcher.prototype.update")

        /* istanbul ignore else */
        if (this.lazy) {
            this.dirty = true;
        } else if (this.sync) {
            // 如果设置了sync标志的话，那么就立即执行
            this.run();
        } else {
            // alert("update through queueWatcher")
            // 否则的话就将watcher加入queue,等待适当时机在执行
            queueWatcher(this);
        }
    };
    ...
    Watcher.prototype.run = function run () {
        if (this.active) {
            //alert(this instanceof Watcher) //true
            //var value = this.get("Watcher's get() in Watcher.prototype.run");
            //alert("value: " + value + ", I am in Watcher.prototype.run");
    ...
    Watcher.prototype.evaluate = function evaluate () {
        this.value = this.get("Watcher's get() in Watcher.prototype.evaluate");
        //this.value = this.get();
        this.dirty = false;
    ...
    function proxy (target, sourceKey, key) {
        sharedPropertyDefinition.get = function proxyGetter () {
             //Print_Trace(arguments)
             //alert("get called I am in proxy " + sourceKey + " " + key)
             return this[sourceKey][key];
        };
        sharedPropertyDefinition.set = function proxySetter (val) {
            //alert("set called I am in proxy " + val)
            this[sourceKey][key] = val;
        };
        /**
         *  The following snippet enables us access keys in original data object
         *  directly through Vue instance.
         */
    ...
    function initData (vm) {
        ...
        //Print_Trace(arguments)
        //此时的vm.$options.data  mergedInstanceDataFn是函数
        ...
        var i = keys.length;
        while (i--) {
            // prop has a higher priority over data.
    ...
    function initComputed (vm, computed) {
        ...
        for (var key in computed) {
            ...
            // callback为啥为noop, 而不是调用callback执行view的相关更新操作？？？？？
...
// without getting yelled at by flow
    ...
    function initRender (vm) {
        ...
        vm.$createElement = function vm_createElement(a, b, c, d) {
     ...
     *     })
     *
     *     Here Vue is actually Vue$3.
     *     Vue$3 only accept one arguement--options object.
     */
    function Vue$3 (options) {
        alert("I am called, in Vue$3!")
     ...
     *             'filter'
     *          ];
     * So, like Vue.component, Vue.directive, Vue.filter
     * is actually call method:
        ...
        ASSET_TYPES.forEach(function (type) {
            Vue[type] = function (
                ...
                if (!definition) {
                    ...
                    // In vue, a componet(and may be directive and filter) is essential a
                    // Vue instance with redefined options.
                    // Because In the following snippet adds definition in Vue's options object.
...
// so they need to be treated as different nodes
    ...
    function createPatchFunction (backend) {
        ...
        function patchVnode (oldVnode, vnode, insertedVnodeQueue, removeOnly) {
            ...
            //  componentInstance:undefined          //  text:undefined
            //  componentOptions:undefined           //  tag:"div"
            //  context:Vue$3                        //  raw:false
            //  data:Object                          //  parent:undefined
            //  elm:div#app.div                      //  ns:undefined
            //  functionalContext:undefined          //  key:undefined
            //  isCloned:false                       //  isStatic:false
            //  isComment:false
            //  isOnce:false                         //  isRootInsert:true


            var data = vnode.data;
            //console.log(JSON.stringify(vnode.data))
            ...
            // console.log(elm);
            // 此时的elm 不包含vue定义的属性，例如下面的元素会被处理为 <input>
            //    <input v-model="message">  调用getAndRemoveAttr
    ...
    // 对于上面的表达式 updateDirective是在Vnode上的操作
    // directives 存放在data区域
    ...
    function _update (oldVnode, vnode) {
        ...
        for (key in newDirs) {
            ...
            if (!oldDir) {
                ...
                // 向dir中添加oldValue属性,以便在dir.def.updata函数中使用
                // 对于v-show来说update函数在 show对象中的update
                dir.oldValue = oldDir.value;
                alert(" divrective: " + dir.name)
    ...
    function getAndRemoveAttr (el, name) {
        ...
        var val;
        // el.attrsMap for quick check.
        // el.attrsList for actual action
        if ((val = el.attrsMap[name]) != null) {
            var list = el.attrsList;
            for (var i = 0, l = list.length; i < l; i++) {
                if (list[i].name === name) {
                    // array deletion using splice
                    // (pos,delCount,addCount)
                    list.splice(i, 1);
                    break
                }
            }
        }
        //alert("name is :" + name + " in getAndRemoveAttr" + " with value : " + val)
        return val
    }
     ...
     *
     * For the following simple v-model application:
     * <input v-model="message">
     *               value is 'message'
     *               assignment is '$event.target.value'
     */
...
// built-in modules have been applied.
    ...
    //
    // 可以作为v-model指令的def属性,在callHook$中调用
    //
    //  platformDirectives
...
// recursively search for possible transition defined inside the component root
    ...
    //
    //  作为v-show指令的def属性,在callHook$1中被调用
    //  platformDirectives
    var show = {
        bind: function bind (el, ref, vnode) {
            var value = ref.value;
       ...
       // Print_Trace(arguments)
        ...
        parseHTML(template, {
            ...
            start: function start (tag, attrs, unary) {
                ...
                // Check the existence of v-pre. if exists, then we can sikp compilation for this element and all its
                // children.
                if (!inVPre) {
                    processPre(element);
                    if (element.pre) {
                ...
                if (inVPre) {
                    ...
                    /**
                     *  process in the following sequence, whether the correspoing
...
// the listener for .once
    ...
    function genHandlers (
        ...
        // 在Vnode节点中data对象生成nativeOn, on的事件处理函数
...
// configurable state
    ...
    function generate (
     ...
     //  上面的HTML对应的code, 加上("with(this){return " + code + "}")就是render [Vue$3 render]
      ...
      //      'div',                                           tagName
      //      {
      //        staticClass:"div",attrs:{"id":"app"}           data object
      //      },
      //      [                                                children
      //    1      _c(
      //             'p',                     tagName
      //              {
      //                directives:[{         data object
      //                  name:"show",
      //                  rawName:"v-show",
      //                  value:(visible),
      //                  expression:"visible"
      ...
      //                            'p',                         tagName
      //                             {
      //                               attrs:{"id":"p"}          data object
      //                              },
      //                              [_v(_s(message))])         children
      //                    : _e(),
      //     4      _v(" "),
      //     5      _c(
      //              'input',             tagName
    ...
    function genElement (el) {
         ...
         // console.log(el)
         // Print_Trace(arguments)
         // called by generate,  genNode,  genTernaryExp
...
// v-once
    ...
    function genIfConditions (conditions) {
        ...
        if (condition.exp) {
            var result = ("(" + (condition.exp) + ")?" + (genTernaryExp(condition.block)) + ":" + (genIfConditions(conditions)));
            //console.log(result)
            return result;
        } else {
    ...
    function genData (el) {
       ...
       //这个data对象是一个Vnode节点的属性,不同的Vnode节点会有不同的data.
       // Print_Trace(arguments) called by genElement
...
// 2: full normalization needed
    ...
    function genText (text) {
        ...
        /**
         * el is a prop array:
         * Array [ "type", "tag", "attrsList", "attrsMap", "parent", "children",
         * "plain", "slotName", "static", "staticRoot" ]
         * If we add some attributes, then attrs will be added to the array.
         */
    function genSlot (el) {
...
// detect problematic expressions in a template
    ...
    function createCompiler (baseOptions) {
        ...
        function compileToFunctions (
            ...
            // check cache
            // 这里为什么以template为key, template可以会很长的(几十万个字符),但这样会不会影响性能
            ...
            // turn code in string form  into functions
            // typeof compiled.render is string

            // console.log(compiled.render)
            // with(this){
            //    return _c('div',{staticClass:"div", attrs:{"id":"app"}},
            //            [_c('p',{directives:[{name:"show",rawName:"v-show",value:(visible),expression:"visible"}]},
            //            [_v(" this is v-show controlled  ")])])
            //    }
